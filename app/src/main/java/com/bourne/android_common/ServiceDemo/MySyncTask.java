...
// * 您可以访问http://www.apache.org/licenses/LICENSE-2.0获取许可证的副本
// *
// * Unless required by applicable law or agreed to in writing, software
// * distributed under the License is distributed on an "AS IS" BASIS,
// * 除非适用法律要求或书面同意，否则根据许可证分发的软件将按“原样”基础分发，
// *
...
// * AsyncTask允许正确和容易地使用UI线程。 此类允许您在UI线程上执行后台操作和发布结果，用或者不用handlers操作线程都可以。
// * <p>
...
// * <p>
// * AsyncTask必须被继承才能使用。 子类将覆盖至少一个方法（{@link #doInBackground}），并且通常会覆盖第二个方法（{@link #onPostExecute}）。
// * <p>
...
// * {@link #onPreExecute（）}，在UI线程上执行任务之前调用。
// * 此步骤通常用于设置任务，例如通过在用户界面中显示进度条。
// * <p>
...
// * 此步骤还可以使用{@link #publishProgress}发布一个或多个进度单位。
// * 这些值在UI线程上的{@link #onProgressUpdate}步骤中发布。
// * <p>
// * <p>
...
// * 为了确保尽快取消任务，您应该始终从{@link #doInBackground（Object []）}定期检查{@link #isCancelled（）}的返回值，如果可能的话 （例如在循环内。）。）
// * <p>
...
// * 从{@link android.os.Build.VERSION_CODES＃DONUT}Android 3.0.x 开始，这被更改为允许多个任务并行操作的线程池。
// * 从{@link android.os.Build.VERSION_CODES＃HONEYCOMB}Android 1.6 开始，任务在单个线程上执行，以避免由并行执行引起的常见应用程序错误。
// * 如果您真的需要并行执行，调用{@link #executeOnExecutor（java.util.concurrent.Executor，Object []）}可以使用{@link #THREAD_POOL_EXECUTOR}。
// */
...
//     * 重写此方法在后台线程上执行计算。
//     * 指定的参数是由此任务的调用者传递给{@link #execute}的参数。
//     * <p>
...
//     * 在{@link #doInBackground}之前,在UI线程上运行
//     *
//     * @see #onPostExecute
//     * @see #doInBackground
//     */
...
//     *               result：由{@link #doInBackground}计算的运算结果。
//     * @see #onPreExecute
//     * @see #doInBackground
...
//     * 通过{@link #publishProgress}传递指定的值。
//     *
...
//     * 在调用{@link #cancel（boolean）}和{@link #doInBackground（Object []）}已经完成后，只能在UI线程上运行。
//     * <p>
//     * <p>The default implementation simply invokes {@link #onCancelled()} and
//     * ignores the result. If you write your own implementation, do not call
//     * <code>super.onCancelled(result)</code>.</p>
//     * 默认实现简单地调用{@link #onCancelled（）}，忽略结果。
//     * 如果你编写自己的实现，不要调用super.onCancelled（result）
//     *
//     * @param result The result, if any, computed in {@link #doInBackground(Object[])}, can be null
//     *               result：在{@link #doInBackground（Object []）}中计算的结果（如果有）可以为null
//     * @see #cancel(boolean)
//     * @see #isCancelled()
//     */
...
//     * <p>
//     * 应用程序应优先重写{@link #onCancelled（Object）}。
...
//     *                              to complete.
//     *                              mayInterruptIfRunning为true时：如果线程执行这一任务应该中断; 否则，正在进行的任务被允许完成。
//     * @return <tt>false</tt> if the task could not be cancelled,
//     * typically because it has already completed normally;
//     * 返回false：如果任务无法取消，通常是因为它已经正常完成;
//     * <p>
...
//     * @throws CancellationException If the computation was cancelled.计算被取消
//     * @throws ExecutionException    If the computation threw an exception.计算抛出异常。
//     * @throws InterruptedException  If the current thread was interrupted
//     *                               while waiting.当前线程在等待期间中断。
//     */
...
//    /**
//     * Executes the task with the specified parameters. The task returns
//     * it self (this) so that the caller can keep a reference to it.
//     * 使用指定的参数执行任务。任务返回它本身（这），以便调用者可以保留对它的引用。
//     * <p>
...
//     * <p>
//     * 警告：允许多个任务从线程池并行运行通常不是想要的，因为它们的操作顺序未定义。
